...
export interface ClassInfo {
  /** Creator Pack 클래스 아이콘의 쪽 번호(1~21). 캐릭터가 이 번호로 가리킨다. */
  icon: number
  name: string
  /** 손에 드는 능력 카드 수. */
  handSize: number
...
export function sanitizeClassInfo(raw: Partial<Row> & { icon: number }): ClassInfo | null {
  if (!hasClassIcon(raw.icon)) return null
...
/** 관리자가 넣는다. 아이콘 번호가 겹치면 덮어쓴다. */
export async function pushClasses(classes: readonly ClassInfo[]): Promise<void> {
...
export async function deleteClass(icon: number): Promise<void> {
  const { error } = await supabase().from('character_classes').delete().eq('icon', icon)
  if (error) throw error
}
