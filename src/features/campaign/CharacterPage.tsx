      ...
      */}
      <header className="topbar" style={{ '--tb': scrolled } as CSSProperties}>
        ...
        {/*
          ...
          처음에는 이름 밑(`topbar__names` 안)에 두었는데, 그러면 배지가 붙을
          때 이름 칸이 길어져 **가운데 맞춤이던 뒤로가기와 표식이 아래로
          밀렸다**(형님이 짚었다). 윗줄 밖으로 내면 윗줄의 키가 변하지 않으므로
          그 둘이 제자리를 지킨다.

          자리만 내주고 그리는 것은 시트가 한다(`chipSlot`) — 고치는 중의 값은
          시트의 초안에 있다.
        */}
    ...
    <section className="joinparty">
      ...
      {usable.length === 0 ? (
        <p className="joinparty__hint">
          들 파티가 없다. <a href="#/journal/new">파티를 생성한다.</a>
        </p>
      ) : (
        <>
