 ...
 *
 * ┌──────────────────────────────────────────────────────────────────────────┐
 * │ **클래스는 여기 없다. 세울 때 정하고 그 뒤로는 못 바꾼다.**                │
 * └──────────────────────────────────────────────────────────────────────────┘
 *
 * 캐릭터가 곧 클래스다 — 레벨·경험·퍽·아이템이 전부 그 클래스에 매인 값이라
 * 클래스만 갈아 끼우면 남은 값들이 통째로 거짓이 된다. 특히 켜 둔 특혜 상자
 * 번호는 그 클래스의 특혜 표를 가리키므로(`0013`), 클래스가 바뀌면 같은 번호가
 * 딴 특혜를 가리키고 그것이 곧 공격 보정 덱의 구성이 된다.
 *
 * **여기서 빼는 것은 보낼 수조차 없게 하는 것이다.** 막는 것은 서버이며
 * (`0014_lock_character_class.sql`) 타입에 남겨 두면 언젠가 다시 새어 든다.
 */
