/**
 * 들고 있는 아이템 하나 — **산 그때의 값을 함께 든다**(`0046`).
 *
 * 값을 상점 목록에서 되찾으면 어긋난다: 평판이 오르내리면 같은 물건을 다른 값에
 * 사고, 목록의 값을 누가 고치면 이미 산 것의 값까지 따라 바뀐다. **산 값은 그때
 * 벌어진 사실**이라 적어 두어야 한다.
 */
export interface OwnedItem {
  readonly name: string
  /** 실제로 낸 값. **옛 줄은 모른다**(`null`) — 짐작해서 채우지 않는다. */
  readonly paid: number | null
  /** 할인 전 값. 낸 값과 같으면 깎인 것이 없다. */
...
export interface Campaign {
  ...
  /**
   * 번영도 눈금에서 **몇 칸이 찼는가**(0~64, `0034`).
   *
   * **레벨은 여기서 셈해서 낸다**(`levelForTicks`) — 캐릭터 레벨을 경험치에서
   * 뽑는 것과 같은 결이다(구현 결정 223). 그 레벨로 상점에 풀리는 아이템 카드가
   * 정해진다.
   *
...
export interface Character {
  ...
  retired: boolean
  /**
   * 지우기로 표시된 시각. 안 눌렀으면 `null`.
   *
   * ┌────────────────────────────────────────────────────────────────────────┐
   * │ **되돌릴 수 없는 일은 곧바로 벌어지지 않는다.**                         │
   * └────────────────────────────────────────────────────────────────────────┘
   *
   * 이틀 뒤에 진짜로 지워진다(`0022`). 그동안은 목록에 그대로 있고 들여다볼 수
   * 있으며 **고칠 수는 없고** 언제든 되돌릴 수 있다.
   */
