 ...
 *
 * ┌──────────────────────────────────────────────────────────────────────────┐
 * │ **일곱은 팩에서 왔고 둘(`p3`·`p4`)은 그 원반을 빌려 구웠다.**              │
 * └──────────────────────────────────────────────────────────────────────────┘
 *
 * 실물 표준 덱에 있는 일곱만 Creator Pack에 있다. `+3`·`+4`는 특혜로만 나오는
 * 값이라 팩에 없는데, 글자로 흘리면 특혜 목록에서 그 줄만 그림이 빠져 어긋나
 * 보인다 — 그래서 팩 `+1`의 원반에서 숫자를 지우고 그 위에 값을 얹어 구웠다
 * (`tools/bake_medallion.py`, ATTRIBUTION에 적혀 있다).
 *
 * **그래서 그림 없는 값을 그리던 갈래는 사라졌다.** 값 낱말은 이 아홉으로
 * 닫혀 있으므로(`VALUE_IDS`) 없는 값이 들어올 길이 없다.
 */
export function valueHasArt(valueId: string): boolean {
  return VALUE_IDS.includes(valueId)
}
