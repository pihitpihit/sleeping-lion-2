...
interface AttackDeckState {
   ...
   */
  reveal: (instanceId: string, composition: DeckComposition) => void
...
export const useAttackDeckStore = create<AttackDeckState>((set, get) => ({
  ...
  reveal: (instanceId, composition) =>
    set((s) => {
      ...
      // 순간의 구성이 맞다.
      const base = s.byInstance[instanceId] ?? createDeck(Math.random, composition)
      const result = drawOne(base, Math.random)
      return { byInstance: { ...s.byInstance, [instanceId]: result.state } }
    }),
