...
//
// NOTICE.md는 vite.config.ts의 플러그인이 빌드 시점에 공개 구간만 남겨서 넘긴다.
// 여기 도착하는 문자열에는 저장소 전용 내용이 이미 없다.
import noticePublicSource from '../../../NOTICE.md?raw'
...
export function NoticePage() {
  const html = useMemo(
    () =>
      marked.parse(rewriteRepoLinks(noticePublicSource), {
        async: false,
        gfm: true,
      }) as string,
    [],
