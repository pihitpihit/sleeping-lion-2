...
| `general/level-crown.svg` | `Character Ability Cards/Ability Cards - Front.jpg`의 위쪽 한가운데 | 캐릭터 레벨 표식 | **크게 변경함** | **아이콘 묶음에는 없어 카드 앞면에서 그 자리를 재어 다시 그렸다**(x 187~223, y 65~87, 37×23px). 그 크기로는 조금만 키워도 흐려지고 확대·JPEG가 가장자리를 물결치게 만들어 **윤곽을 그대로 따지 않았다** — 봉우리·골·어깨 끝·홈의 자리를 화소 단위로 재고, 아래 원호·옆면 곡선·위쪽 오목 곡선 넷이라는 얼개로 다시 그렸고, 홈 셋은 테와 같은 폭만큼 물려 짓는다. 자리는 원본 그대로이고 곡선은 우리가 그린 것이다. 재는 스크립트는 `tools/extract_crown.py`, 그리는 스크립트는 `tools/draw_crown.py` |
| `general/hp-drop-lit.webp` | `general/hp-drop.svg`(같은 파일 8쪽)에서 파생 | HP/XP 트래커의 체력 표식 | **크게 변경함** | 〃 (붉은 쪽) |
| `general/hp-drop-deep-lit.webp` | 〃 | 어두운 바탕(설정 목록·로그)의 체력 표식 | **크게 변경함** | 〃 (붉은 쪽, 진하게) |
