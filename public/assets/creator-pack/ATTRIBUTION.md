...
> **2026-08-12 — 이제 필요해졌는데 못 가져오고 있다.** 공격 보정 카드에 상태이상
> 표식(부상·이동불가·중독·혼란·기절·저주·무장해제·투명)과 수치 표식(밀기·당기기·
> 관통·치료·방어·대상 추가)을 그려야 하는데, 그 그림들이 `General Icons.pdf`의
> 안 가져온 쪽에 있다. **팩 원본이 작업 기계에 없어 뽑지 못했다.** 그때까지는
> 배지 안에 한 글자를 적어 대신한다 — 흉내 내 그리지 않는다. 팩을 다시 구하면
> 여기 표에 줄을 더하고 글자를 그림으로 바꾼다. 팩의 `Font/` 디렉토리는 저작자가 라이선스할 수 있는
